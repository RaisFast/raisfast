...
-- A conversation session bound to an agent + owner. `parent_id` links debate
-- child sessions to their origin (multi-agent §5, NULL = top-level session).
...
-- `ledger` is the dispute-ledger JSON (single source of state); rounds append
-- to `rounds_done` + `rounds_log` inside the ledger. `origin_session_id`
-- anchors session-scoped debates (NULL = cold-start via POST /debates).
...
CREATE TABLE IF NOT EXISTS llm_channels (
    ...
    cost_discount DOUBLE PRECISION NOT NULL DEFAULT 1.0,
    monthly_cost DOUBLE PRECISION,
    test_model VARCHAR(255),
    test_time TIMESTAMPTZ(0),
...
CREATE TABLE IF NOT EXISTS llm_models (
    ...
    input_price DOUBLE PRECISION NOT NULL DEFAULT 1.0,
    output_price DOUBLE PRECISION NOT NULL DEFAULT 1.0,
...
-- Archived daily rollup of llm_logs (design §9 retention / §16 汇总表): the
-- archive job upserts aggregates here, then clears the detail rows, so
-- long-term reports survive detail deletion. Grain = day × source × model ×
-- channel × user (the finest dimension `stats_by` can group by).
CREATE TABLE IF NOT EXISTS llm_logs_summary (
    ...
    cost_quota BIGINT NOT NULL DEFAULT 0,
    updated_at TIMESTAMPTZ(0) NOT NULL DEFAULT NOW(),
    UNIQUE(tenant_id, day, source, model_name, channel_id, user_id)
);
CREATE INDEX IF NOT EXISTS idx_llm_logs_summary_day ON llm_logs_summary(tenant_id, day);
...
CREATE TABLE IF NOT EXISTS docparse_usage_summary (
    ...
    UNIQUE(tenant_id, kind, engine, date)
);
